...
app.get("/posts/:title", function(req, res) { //using express routing parameters.
  const newPostTitle = _.lowerCase(req.params.title);
  posts.forEach(function (post){
    const titleOfLastPost = _.lowerCase(post.titleBody);
    const bodyOfLastPost = post.postBody; //post is the js object defined in app.post("/compose") in which postBody is a key
    if (titleOfLastPost === newPostTitle) {
      res.render("post", {newPageTitle : post.titleBody ,newPageContent : bodyOfLastPost});
    }
  })
// [posts.length - 1]
});
...
app.post("/compose", function(req, res) {
  ...
  let newTitle = req.body.TitleMssg;
  let newContent = req.body.PostMssg;
  let post ={
    titleBody : newTitle,
    postBody : newContent
  }
